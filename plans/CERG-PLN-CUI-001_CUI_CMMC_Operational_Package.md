...
The Controlled Unclassified Information (CUI) Handling Standard names what is required; this package makes the standard executable. It assembles the System Security Plan (SSP), Plan of Action and Milestones (POA&M), Supplier Performance Risk System (SPRS) worksheet, boundary diagrams, data flow maps, category register, 800-171 evidence matrix, [Cybersecurity Maturity Model Certification (CMMC) Level 2](https://dodcio.defense.gov/CMMC/) readiness checklist, CMMC Third-Party Assessment Organization (C3PAO) logistics, subcontractor register, and FedRAMP equivalency evidence into a single operational binder.
